...
namespace DocGeneratorCore.Database.Classes
	...
	public class ElementDeliverable : OptimizedPersistable
		...
		[Index]
		private int? _AssociatedElementIDsp;
		#endregion
		...
		public int? AssociatedElementIDsp {
			get { return this._AssociatedElementIDsp; }
			set { Update(); this._AssociatedElementIDsp = value; }
			}
		#endregion
		...
		public static bool Store(
			...
			int parAssociatedElementIDsp
			)
			{
			ElementDeliverable newEntry;
			try
				{
				using (ServerClientSession dbSession = new ServerClientSession(systemDir: Properties.Settings.Default.CurrentDatabaseLocation))
					...
					newEntry = (from objEntry in dbSession.AllObjects<ElementDeliverable>()
								where objEntry.IDsp == parIDsp
								select objEntry).FirstOrDefault();
					if (newEntry == null)
						newEntry = new ElementDeliverable();
					...
					newEntry.AssociatedElementIDsp = parAssociatedElementIDsp;
				...
				{
				Console.WriteLine("### Exception Database persisting ElementDeliverable ### - {0} - {1}", exc.HResult, exc.Message);
				return false;
				}
			}
		//---G
		...
		public static ElementDeliverable Read(int parIDsp)
			{
			ElementDeliverable result = new ElementDeliverable();
			try
				{
				using (ServerClientSession dbSession = new ServerClientSession(systemDir: Properties.Settings.Default.CurrentDatabaseLocation))
					{
					...
					result = (from thisEntry in dbSession.AllObjects<ElementDeliverable>()
